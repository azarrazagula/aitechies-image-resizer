...
const getPlatformIcon = (category: string) => {
  switch (category) {
    case "Instagram":
      return (
        <svg className="w-4 h-4 xs:w-5 xs:h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
          ...
          {PLATFORMS.map((platform) => {
            ...
            return (
              <motion.button
                ...
                className={`w-9 h-9 xs:w-10 xs:h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-xl border transition-all duration-300 flex-shrink-0 ${
                  isSelected
