...
export default function CanvasPreview({
  ...
  const canvasRef = useRef<HTMLCanvasElement>(null); // Resized canvas
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      if (mode === "fit") {
        ...
        ctx.drawImage(
          img,
          (targetW - img.width * scale) / 2 + cropOffset.x,
          (targetH - img.height * scale) / 2 + cropOffset.y,
          img.width * scale,
          img.height * scale,
        );
      } else {
  ...
  const handleMove = (clientX: number, clientY: number) => {
    ...
    const scale = Math.max(
      targetW / imageMeta.width,
      targetH / imageMeta.height,
    );
    ...
    onCropOffsetChange({
      x: Math.max(
        -maxDevX,
        Math.min(maxDevX, dragStart.current.offsetX + dx * ratioX),
      ),
      y: Math.max(
        -maxDevY,
        Math.min(maxDevY, dragStart.current.offsetY + dy * ratioY),
      ),
    });
  };
  const handleEnd = () => {
  ...
  const maxHClass =
    "max-h-[200px] min-[375px]:max-h-[240px] min-[410px]:max-h-[280px] sm:max-h-[340px] md:max-h-[400px]";
  ...
  if (showOriginal && imageMeta && targetW && targetH) {
    ...
    if (absoluteMax >= baseColumnWidth) {
      ...
      if (origDominant >= destDominant) {
        ...
        resizedWidthStyle = {
          width: `${Math.max(35, (destDominant / origDominant) * 100)}%`,
        };
      } else {
        // Resized is larger
        originalWidthStyle = {
          width: `${Math.max(35, (origDominant / destDominant) * 100)}%`,
        };
  ...
  // Resized canvas container style (centered flex container to size container matching image width)
  ...
  return (
    <div className="flex flex-col items-center gap-3 w-full">
      {/* ══════════════════════════════════════════════════════
          RESPONSIVE LIVE PREVIEW: Toggleable Original + Resized
          ══════════════════════════════════════════════════════ */}
      <div className="flex flex-col gap-3 w-full">
        ...
        <div className="flex items-center justify-between px-1">
          ...
          <span
            className={`text-[10px] sm:text-xs font-mono px-2.5 py-1 rounded-lg border transition-all duration-300 ${
              isTransitioning
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full overflow-hidden">
          ...
          <AnimatePresence>
            {showOriginal && (
              <motion.div
                ...
                className="flex flex-col gap-2 min-w-0 overflow-hidden w-full md:w-1/2">
                {/* Original label row */}
                ...
                <div
                  className="relative border border-neutral-800/60 rounded-2xl overflow-hidden shadow-lg mx-auto w-full"
                  style={{
                    ...checkerStyle,
                    ...originalWidthStyle,
                  }}>
          ...
          <motion.div
            ...
            className={`flex flex-col gap-2 min-w-0 ${
              showOriginal ? "w-full md:w-1/2" : "max-w-[450px] w-full mx-auto"
            }`}>
            {/* Resized label row */}
            <div className="flex items-center justify-between px-0.5">
              <span
                className={`text-[10px] sm:text-xs font-bold uppercase tracking-widest flex items-center gap-1.5 transition-colors duration-300 ${
            ...
            <motion.div
              ...
              <canvas
                ...
                className={`block select-none transition-opacity duration-300 max-h-[300px] sm:max-h-[380px] md:max-h-[60vh] lg:max-h-[65vh] ${
            ...
            {!showDimEdit ? (
              <motion.div
                ...
                <span className="flex items-center gap-1 lg:gap-1.5">
                  <span className="text-neutral-500 text-[10px] lg:text-[11px] font-bold">
                    ↔
                  </span>
                  ...
                  <span className="text-neutral-600 text-[9px] lg:text-[10px]">
                    px
                  </span>
                </span>

                <span className="text-neutral-700 font-bold text-xs lg:text-sm">
                  ×
                </span>
              ...
              >
                ...
                <label className="flex items-center gap-1 bg-[#111]/60 border border-neutral-800/80 rounded-xl px-1.5 lg:px-2.5 py-1 lg:py-1.5 focus-within:border-[#8B5CF6]/50 transition-colors">
                  ...
                  <input
                    ...
                    autoFocus
                  />
                  <span className="text-neutral-600 text-[9px] lg:text-[10px] select-none">
                    px
                  </span>
                </label>
      ...
      <div className="flex items-center justify-between w-full max-w-sm px-5 py-3.5 bg-[#161616]/60 border border-white/5 rounded-2xl shadow-inner select-none">
        <div className="flex flex-col items-start gap-0.5">
          ...
          <span className="font-mono text-xs sm:text-sm text-neutral-300 font-semibold">
        ...
        <div className="flex flex-col items-end gap-0.5">
          ...
          <span className="font-mono text-xs sm:text-sm text-[#8B5CF6] font-bold">
            <span
              className={`transition-all duration-300 inline-block ${isCalculatingSize ? "opacity-50 scale-95 blur-[0.5px]" : "opacity-100 scale-100"}`}>
